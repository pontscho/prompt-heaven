...
async def handle_luals_call(args: dict, server: Optional["McpServer"] = None) -> str:
    ...
    def _serialize(fn: str, data: Any) -> str:
        ...
        # Read off `data` while it is still a dict — see _ErrorText. Every
        # failure this server can report passes through here, the broad
        # `except Exception` below included: it converts a raise into
        # {"error": ...} rather than letting it reach the wrap, which is
        # precisely why the wrap's own `except` cannot be where the flag is set.
