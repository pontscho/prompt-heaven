...
# `_format_search_results` below — the pager that actually decides both.
# Refresh: python3 Scripts/amalgamate.py -- do not edit inside the region (§8).
...
def _format_search_results(results: list, offset: int = 0,
    ...
    The ``offset`` handed in is the RECORD index `_offset` read off the wire, and
    the paging is display-level over the record list this call already fetched,
    not an upstream cursor: the search is re-run on every call. Only
