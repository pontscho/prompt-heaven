...
GIT_CALL_TOOL = {
    ...
    "description": (
        ...
        "  Bash call. Pass the message via a HEREDOC so multi-line messages,\n"
        "  backticks, quotes, and `$` survive shell expansion untouched.\n\n"
        ...
        "    - The HEREDOC above is the ONLY message channel. Do NOT write the\n"
        "      message to a file and read it back with `-F`/`--file=`/`-F -`,\n"
        "      and do NOT `Write`/`create_text_file` a scratch commit-message\n"
        "      file (e.g. `.claude/tmp/commit-msg.txt`) first. The message must\n"
        "      never touch disk — a file detour is slower, leaks a stray\n"
