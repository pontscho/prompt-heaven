...
PURITY_CALL_TOOL = {
    ...
    "description": (
        ...
        "NEVER use find, grep, awk, sed, python scripts, fd, or any ad-hoc Bash "
        "one-liners for file searching, file listing, or in-place editing. "
        "`find` in particular is FORBIDDEN — `find_file` covers every Unix "
        "`find -name <glob>` / `find <dir> -name <glob>` / `find -type f` "
        "wildcard search, returns sandbox-scoped relative paths, skips `.git`, "
        "and supports pagination via `head_limit` / `offset`. The pattern is "
        "fnmatch-style (`*.cu`, `test_*.py`, etc.), and the search root is the "
        "optional `path` (alias of `relative_path`, default = project root). "
        "If you catch yourself reaching for Bash(\"find ...\"), STOP and call "
        "`find_file` instead — that's what it exists for.\n\n"
