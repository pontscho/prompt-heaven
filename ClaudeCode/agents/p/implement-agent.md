...
description: `Execute planned implementations from YAML documents generated by /p:task-plan. Reads requirements.yaml, validates completeness, executes tasks in dependency order following code_references patterns exactly, runs tests after each task, tracks progress with automatic status updates. Use when implementing features from task plans, executing implementation plans, following structured YAML-based development workflows, or resuming interrupted implementations. Trigger conditions: implement the plan; execute task-plan; run implementation; continue implementation; implement from requirements.yaml.`
...
1. **Language-specific code quality checks**:
   - **For C/C++ files**: Run `cd <to-project-root> ; clang-tidy -p build <file_path>` and fix all warnings
   - **For Lua files**: Check syntax with `luac -p <file_path>`
   - **For Python files**: Run relevant linters if configured (e.g., pylint, flake8)
2. **Build**: Run `cd <to-project-root> ; cmake --build build` to verify compilation
