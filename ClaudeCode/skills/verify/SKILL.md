---
name: verify
description: Validate that a file is FORMALLY well-formed (parses) for its format — JSON, Python, YAML, TOML, XML, INI/.cfg, CSV/TSV, plist, JavaScript, Bash/sh. Use when asked "is this valid JSON/YAML/Python/JavaScript/shell", to check a config/data file or script before committing or deploying, to verify a file you just wrote or generated, or as a pre-commit/CI format gate. Bundles a Python 3.9 stdlib-first validator at scripts/validate.py for shell and CI use (JavaScript and Bash additionally need the node and bash binaries — syntax check only, never executed); inside a Claude session prefer the pre-approved MCP tool inspect_call (function validate/json/python/yaml/...), which costs no permission prompt. This is FORMAT/structural validation (does it parse), NOT schema validation (does it match an expected shape).
---
...
All eleven formats are exposed by the pre-approved `mcp-inspect` server: no
permission prompt, a Markdown table back, and a batch of files in one call. Two
of them need an external **binary** in PATH and FAIL rather than skip without it:
JavaScript (`node --check`) and Bash (`bash -n`). `sh` and `shell` are accepted
spellings of `bash` — one parser, and every verdict says `bash -n` so nothing
lies about which one ran.

The bundled script covers the **same eleven formats**; the two paths differ only
in *interface*, never in coverage (the MCP path adds batching via `paths`, inline
`content`, a `max_mb` cap and a Markdown table — the script takes files and stdin
...
inspect_call {function: "bash",     params: {path: "hooks/post-edit-lint.sh"}}  # bash -n
```
...
The bundled script covers all eleven formats for pre-commit hooks, `make`
...
python ~/.claude/skills/p/skills/verify/scripts/validate.py deploy.sh               # bash -n
echo '{"a":1}' | python ~/.claude/skills/p/skills/verify/scripts/validate.py --format json -
```

Format is auto-detected from the extension; override with `--format` (which also
accepts `sh` and `shell` for `bash`). Use `-` as a path to read stdin (then
`--format` is required). One line of output per file; a one-line summary goes to
stderr.
...
| `FAIL`    | Does not parse. Message + `line:col` of the first error (`bash -n` reports a line but never a column). **Also**: JavaScript or Bash was asked for and `node` / `bash` is not in PATH. |
| `LIMITED` | YAML checked by the stdlib pre-check only (no PyYAML), or a `node --check` / `bash -n` that timed out. Either a real defect was found (with a line) or only the conservative checks passed — **not a full parse.** |
...
a missing `node` or `bash` **fails**. Deliberate: those two are optional *parsers*
...
| Bash / sh  | `bash -n` (external binary) | **Syntax only** — not shellcheck, not linting. The script is **never executed**: `-n` is bash's own read-but-do-not-execute mode, and `bash -c`, `bash -i`, `source` and a plain `bash <file>` are never used, nor is a temp file ever written. `.sh`/`.bash` auto-detect; `.zsh`/`.fish`/`.ksh` deliberately do **not** (bash reports those dialects' own syntax as a bogus error), and an extension-less script on PATH has nothing to infer from — pass `--format bash` there. `sh` and `shell` are the same parser, so a POSIX script carrying a bashism can come back `OK` (a false OK) but a correct one can never come back `FAIL`: bash's grammar is a superset of sh's. One line, never a column. **No `bash` in PATH → `FAIL`.** |
...
JavaScript and Bash have no stdlib equivalent at all — by hand they are
`node --check file.js` and `bash -n file.sh` (exit 0 = parses). Never `node -e`/`-p`
or a require, and never `bash -c` or a bare `bash file.sh`: those would *run* the
file. Inside a session use `inspect_call {function: "javascript"}` /
`inspect_call {function: "bash"}` instead; a guard denies the Bash form of both
...
- `--list-formats` prints the supported formats — the same eleven `inspect_call`
  exposes. `sh` and `shell` are accepted by `--format` but are not listed: they
  are spellings of `bash`, not formats of their own.
- The script is dependency-free on JSON/Python/XML/INI/CSV/plist. TOML needs a
  ...
  binary and Bash the `bash` binary. No pip dependency in any case.
- `.py` and `.pyi` auto-detect as Python, `.js`/`.mjs`/`.cjs` as JavaScript,
  `.sh`/`.bash` as Bash; force a format elsewhere with `--format python` /
  `--format javascript` / `--format bash`.
