...
if [ -n "$files" ]; then
    for file in $files; do
        ~/.claude/skills/p/skills/cmake/cmake-validator.py "$file"
        if [ $? -ne 0 ]; then
...
- name: Validate CMake files
  run: |
    pip install --no-deps ~/.claude/skills/p/skills/cmake/cmake-validator.py
    find . -name "CMakeLists.txt" -o -name "*.cmake" | xargs ~/.claude/skills/p/skills/cmake/cmake-validator.py
```
