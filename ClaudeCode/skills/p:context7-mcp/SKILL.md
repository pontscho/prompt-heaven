...
# Context7 MCP Server — Full API Reference
...
All Context7 operations go through `context7_call(function=..., params={...})`.
This keeps the tool list minimal while giving full access to the documentation API.

## How to call any Context7 function

```
mcp__context7__context7_call(
...
```
mcp__context7__context7_call(function="context7_resolve_library_id", params={
  "query": "react hooks",
  "library_name": "react"
})
```
...
```
mcp__context7__context7_call(function="context7_query_docs", params={
...
Skip `context7_resolve_library_id` if the user already provides a library ID in the
format `/org/project` or `/org/project/version` (e.g. `/vercel/next.js/v14.3.0`).
...
Call this before `context7_query_docs` unless the user already provides a library ID.

```json
{
  "query":        "The question or task you need help with — used to rank results by relevance.",
  "library_name": "Library name to search for (e.g. \"react\", \"next.js\", \"pandas\")"
}
```
...
```json
{
  ...
  "query":      "Specific question or task — be detailed. E.g. 'How to set up JWT auth in Express.js'"
}
```
...
**Query guidance:** Be specific. Good: `"useEffect cleanup for subscriptions"`. Bad: `"hooks"`.
Do not call this more than 3 times per question — use the best result you have.
...
**Send multiple `context7_call`s in a single response** when you need docs for several
libraries at once — only ONE model API round-trip is needed instead of N.
