...
### Sizing content-rich boxes (title + bullets)

For boxes with structured multi-line content (bold title + italic subtitle + bullet list), use `verticalAlign=top` with `align=left` and `spacingLeft=10;spacingTop=8;spacingRight=8;`. The content then anchors to the top-left corner of the box.

**Right-size the box height** — `verticalAlign=top` leaves visible empty space at the bottom when the box is taller than the content. Estimate:

```
box_height ≈ rendered_lines × 16 + 20    (16 px line, 20 px total padding)
```

Where `rendered_lines` counts wrap-induced lines too. Estimate wraps at ~7 px per character (proportional font): a line of `N` chars at width `W` wraps to `⌈N / (W/7)⌉` lines. Example: width 320 px content area ≈ 45 chars/line.

**Common mistake**: over-sizing boxes "to be safe" — bullet content sits at the top, half the box stays empty, the diagram looks unprofessional. If you cannot estimate accurately, prefer a slightly tighter box and accept that the last bullet may sit one pixel below center: better than 30 px of unused space.

If a box has a mix of one-liners and a long wrapping line, the long line dominates — count it as `⌈chars / 45⌉` lines.

## Highlighting a chosen / primary path

When a diagram compares options or shows several alternative flows, mark the chosen one by combining three signals — never just one (a single signal reads as "decorative" rather than "selected"):

1. **Node fill + stroke** on every node along the path: `fillColor=#d5e8d4;strokeColor=#82b366;` (green family)
2. **Edge color + weight** between those nodes: `strokeColor=#82b366;strokeWidth=2;`
3. **Lane background** (if using swimlanes): use a stronger pastel for the chosen lane (`#e8f5e9` green) while the alternatives stay neutral (`#f5f5f5`, `#e8f4f8`, `#fff0e6`)

Matched `fillColor` / `strokeColor` pairs — pick by semantics:

| Semantics | fillColor | strokeColor |
|-----------|-----------|-------------|
| Chosen / positive | `#d5e8d4` | `#82b366` |
| Primary / info | `#dae8fc` | `#6c8ebf` |
| Warning / attention | `#fff2cc` | `#d6b656` |
| Excluded / negative | `#f8cecc` | `#b85450` |
| Special | `#e1d5e7` | `#9673a6` |

Do NOT colorize text (`fontColor`) for emphasis — colored labels reduce legibility against pastel fills. Reserve `fontColor` for dark-mode overrides only.
...
### Edge label placement (avoid overlap)

Edge labels render mid-edge by default. Multi-line labels can visually overlap the source or target node when the gap between them is too small — this looks broken even when the underlying geometry is correct.

**Always add `labelBackgroundColor=#ffffff` to any edge that has a `value` (label).** This gives the label its own opaque card so it cleanly sits on top of any shape it crosses. Cheap, defensive, and never makes anything worse. Optionally pair with `labelBorderColor=#bbbbbb` for a subtle outlined card.

**Vertical gap budget** for orthogonal edge labels (default 12 px font, ~16 px line height):

| Label lines | Required gap between nodes |
|-------------|----------------------------|
| 1 line | ≥ 30 px |
| 2 lines | ≥ 50 px |
| 3 lines | ≥ 70 px |
| 4+ lines | ≥ 90 px, OR shorten the label |

When the label is unavoidably long, prefer **shortening the wording** ("(connect, releaseStream, FCPublish, createStream, publish)" → "(connect, releaseStream, publish, ...)") over fighting the geometry. Long edge labels are usually a sign that detail belongs inside one of the connected nodes, not on the arrow.
...
- **Lane title length: keep it ≤ ~18 characters.** With `horizontal=0` the title is rendered rotated 90° along the lane height (150 px). Default 12 px font, vertical title text runs the full lane height — anything longer wraps and bleeds into adjacent lanes. Strip trailing parentheticals ("Hosted MCP App Server (inline iframe)" → "Hosted MCP App"). If a longer title is unavoidable, raise the lane height to 200–250 px and adjust every lane uniformly.
