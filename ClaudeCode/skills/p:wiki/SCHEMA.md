...
`INDEX.md` is regenerated by `scripts/reindex.py`, never hand-written.
...
`concept`, `runbook`. They are optional for `overview`, `adr`, `glossary`.
...
detected cheaply by `scripts/freshness.py` with git only. Symbol-level checks
...
5. Run `scripts/reindex.py` to refresh `INDEX.md`.
...
1. Run `scripts/freshness.py` -> prose report listing stale / unverified / orphaned-source pages.
2. Run `scripts/reindex.py --check` -> orphans, dup slugs, malformed frontmatter.
...
- Git freshness (did sources change since a commit) -> git MCP / `scripts/freshness.py`.
