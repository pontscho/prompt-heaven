---
name: sandbox-run
description: Run a shell command inside an OS-level sandbox (default-deny writes, no network) via the bundled `sbx` wrapper -- macOS Seatbelt or Linux bwrap, fail-closed on any other platform or missing sandbox binary. Parses `--write DIR` (repeatable), `--net`, `--ro`, and `--dry-run` before a bare `--` and execs the target under the sandbox. Paired with the `sbx-gate.py` PreToolUse(Bash) hook, which auto-allows -- without a permission prompt -- a clean, in-project, network-free invocation of the ONE deployed wrapper, matched by canonical-path IDENTITY (never by filename). Registration of the gate is manual and the user's responsibility; there is no installer.
model: sonnet
---
