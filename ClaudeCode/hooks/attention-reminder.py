...
MINION_MINDSET_BLOCK = (
	...
	"- You wrote an implementation plan → `p:minion-inspector-plan` then `p:minion-inspector-security-officer` (plan-mode)\n"
	"- You finished implementing → `p:minion-inspector-implementation` then `p:minion-inspector-security-officer` (code-mode)\n"
...
FIRST_REMINDER_TEMPLATE = (
	...
	"- Bash is the LAST RESORT, never the first reach. When an MCP covers the "
	"domain, reaching for `Bash` is a VIOLATION — route it instead: read-only "
	"`git` (status / log / diff / show / blame / stash) → `git_call`; build / "
	"test / clean (`make` / `cmake` / `ctest` / `npm` / `cargo` / `ninja`, when "
	"`project-forge.yaml` exists) → `forge_call`; `grep` / `find` / `sed` / "
	"`awk` / `cat` / `head` / `tail` / `ls` → `purity_call` (search_for_pattern "
	"/ find_file / read_file / replace_content). Piping or chaining "
	"(`|`, `&&`, `;`, `xargs`) does NOT make a banned bash command acceptable.\n"
