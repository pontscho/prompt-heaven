...
**Bounded nesting (depth-2 max).** Sub-agent nesting was long treated as unsupported; it is now **verified working in this harness to ≥3 levels** (verified 2026-07, incl. a custom minion with an `Agent` grant spawning a leaf child). We nonetheless cap it deliberately. An **executor minion** — a worker that carries a unit of production work end-to-end, currently exactly two: `p:minion-mason` (one planned task) and `p:minion-bug-hunter` (one reported bug) — MAY spawn **leaf workers** to offload token-heavy sub-tasks (bug investigation, codebase exploration, noisy reproduction runs, an independent verdict on its own claim) and keep its own context lean. Each executor declares a **fixed child allowlist** in its own file: mason may spawn `p:minion-watson` + `p:minion-explorer`; bug-hunter may spawn `p:minion-watson`, `p:minion-explorer`, `p:minion-runner` + `p:minion-code-verifier`. Its children MUST be leaf workers, so the chain is `main/skill → executor → leaf → (stop)`: a hard **depth-2 ceiling**. This bounds exactly the cost/context-opacity the original no-nesting rule guarded against. **Everything that is not a designated executor stays a leaf** — planners, inspectors, reviewers, verifiers, builder, runner, explorer, watson, and all other minions NEVER spawn sub-agents. Orchestration of *workflows* still lives in skill/main context, never inside a minion. **Being an executor is not a licence to orchestrate:** it carries ONE unit of work and offloads sub-tasks *within* it. The moment a job spans several units, needs ordering between them, or needs an interactive decision, it is a workflow and belongs in a skill (see *Skill vs Agent* below) — not in a minion.
...
**Leaf-worker** agents MUST NOT list `Agent` in their `tools:`. Only a designated **executor minion** may list `Agent`, and then ONLY to spawn leaf workers from a declared allowlist (see the bounded-nesting rule above) — currently there are exactly two executors: `p:minion-mason` (allowlist: `p:minion-watson`, `p:minion-explorer`) and `p:minion-bug-hunter` (allowlist: `p:minion-watson`, `p:minion-explorer`, `p:minion-runner`, `p:minion-code-verifier`). `Skill` is permitted for any minion because the `Skill` tool only loads instructions into the *current* context (no new sub-agent is spawned), so it does not breach the nesting rule — a minion may legitimately invoke a passive instruction skill to specialize its own behavior. Use sparingly.
...
| Bug investigation (diagnosis only, read-only) | `p:minion-watson` |
| Closing a reported bug end-to-end (reproduce → diagnose → fix → prove) | `p:minion-bug-hunter` |
